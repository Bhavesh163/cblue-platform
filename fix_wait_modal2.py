...
# Since `onOrderClick` is optional to avoid typescript yelling if we missed one:
text = text.replace(
    'onClick={() => window.location.href = `${prefix}/chat/${o.id}`}',
    'onClick={() => onOrderClick ? onOrderClick(o) : window.location.href = `${prefix}/chat/${o.id}`}'
)

# 4. Insert Modal at the end of CustomerDashboard
...
# Last brace of CustomerDashboard
