        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full mx-4 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              {locale === "th" ? "กรุณาเข้าสู่ระบบก่อน" : locale === "zh" ? "请先登录" : "Please Log In First"}
            </h2>
            <p className="text-gray-600 mb-6">
              {locale === "th"
                ? "คุณต้องเข้าสู่ระบบหรือลงทะเบียนก่อนจึงจะส่งคำขอจองได้"
                : locale === "zh"
                  ? "您需要登录或注册后才能提交预约请求"
            ...
            <div className="flex gap-3 justify-center">
              <Link
                href={`${prefix}/login`}
                className="px-6 py-2.5 bg-sky-600 text-white rounded-lg font-medium hover:bg-sky-700 transition"
              >
              ...
              >
                {locale === "th" ? "ลงทะเบียน" : locale === "zh" ? "注册" : "Register"}
              </Link>
            </div>
            <button
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8 space-y-6">
          ...
          <fieldset className="bg-gradient-to-r from-sky-50 to-blue-50 border-2 border-sky-200 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-5 gap-3">
              {[
                { value: "economy", label: locale === "th" ? "ประหยัด" : locale === "zh" ? "经济型" : "Economy", deposit: "฿200", emoji: "🟢", stars: "⭐", desc: locale === "th" ? "ทั่วไป" : locale === "zh" ? "普通" : "Basic" },
