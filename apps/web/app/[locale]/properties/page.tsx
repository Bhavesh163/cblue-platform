...
const CORE_PROPERTY_TYPE_SET = new Set<string>(
  CORE_PROPERTY_TYPES as readonly string[],
);
...
function hasValidGpsCoordinatePair(
  latitude: number | null,
  longitude: number | null,
) {
...
async function downloadPropertyFiles(
  urls: string[],
    ...
    try {
      ...
      const token =
        typeof window !== "undefined"
          ? localStorage.getItem("subscriber_token") || ""
          : "";
...
function sanitizeProperty(raw: any): Property {
  ...
  return {
    ...
    locationMode:
      String(raw?.locationMode || "").toUpperCase() === "GPS"
        ? "GPS"
        : "ADMINISTRATIVE",
...
function dedupeProperties(items: Property[]) {
  ...
  for (const item of items) {
    ...
    const existingHasUsableLocation =
      getPropertySummaryLocation(existing) !== "Unknown";
    const nextHasUsableLocation =
      getPropertySummaryLocation(item) !== "Unknown";
    const existingQuality =
      (existingHasImage ? 2 : 0) + (existingHasUsableLocation ? 1 : 0);
    const nextQuality =
      (nextHasImage ? 2 : 0) + (nextHasUsableLocation ? 1 : 0);
    const existingTs = parsePropertyTimestamp(
    ...
    if (
      ...
      (nextQuality === existingQuality && nextTs >= existingTs)
    ) {
      bestByKey.set(key, item);
    }
  }
  ...
  return Array.from(bestByKey.values()).sort(
    (a, b) =>
      parsePropertyTimestamp(b.createdAt) - parsePropertyTimestamp(a.createdAt),
  );
}
...
function PropertiesPageContent() {
  ...
  const [pendingContactProp, setPendingContactProp] = useState<Property | null>(
    null,
  );
  const [showContactFlow, setShowContactFlow] = useState<Property | null>(null);
  const [contactStep, setContactStep] = useState<"po" | "notify" | "done">(
    "po",
  );
  ...
  useEffect(() => {
    ...
    const openFromQuery = async () => {
      let target =
        [...properties, ...latestProperties].find(
          (item) => item.id === autoContactId,
        ) || null;

      if (!target) {
        try {
          const res = await fetch(
            `/api/v1/properties/${encodeURIComponent(autoContactId)}`,
          );
          if (res.ok) {
  ...
  async function handleSearch(overrides?: Partial<typeof filters>) {
    ...
    try {
      ...
      const selectedPropertyType = String(f.propertyType || "")
        .trim()
        .toUpperCase();
      if (
        selectedPropertyType &&
        CORE_PROPERTY_TYPE_SET.has(selectedPropertyType)
      ) {
  ...
  async function getInquirySessionType(
    authToken: string,
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500">
                {locale === "th"
                  ? "เข้าสู่ระบบหรือสมัครสมาชิกเพื่อติดต่อผู้ลงประกาศ"
                  : locale === "zh"
                    ? "登录或注册以联系发布者"
                    : "Log in or register to contact the property lister"}
              </p>
            </div>
            {/* Toggle */}
            <div className="flex gap-2 mb-4">
              <button
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition ${authMode === "login" ? "bg-green-700 text-white" : "bg-gray-100 text-gray-600"}`}
              >
                {locale === "th"
                  ? "เข้าสู่ระบบ"
                  : locale === "zh"
              ...
              <button
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition ${authMode === "register" ? "bg-green-700 text-white" : "bg-gray-100 text-gray-600"}`}
              >
                {locale === "th"
                  ? "สมัครสมาชิก"
                  : locale === "zh"
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                placeholder={
                  locale === "th"
                    ? "รหัสผ่าน (ตัวใหญ่+เล็ก+ตัวเลข+อักขระพิเศษ)"
                    : locale === "zh"
                      ? "密码（大小写+数字+特殊字符）"
                      : "Password (A-z + 0-9 + !@#)"
                }
              ...
              {authMode === "register" && (
                <label className="flex items-start gap-2 rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600">
                  <input
                    ...
                    onChange={(e) => {
                      setAuthPdpaConsent(e.target.checked);
                      setAuthError("");
                    }}
              ...
              <button
                onClick={async () => {
                  ...
                  if (
                    authMode === "register" &&
                    !/^[0-9\s+()\-]{9,15}$/.test(authPhone.trim())
                  ) {
                    setAuthError(ts("invalidPhone"));
                    return;
                  }
                  if (authMode === "register" && !authPdpaConsent) {
                    setAuthError(ts("pdpaRequired"));
                    return;
                  }
                  try {
                    const endpoint =
                    ...
                    const inquirySession = await getInquirySessionType(
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-green-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
              ...
              <button
                onClick={() => setShowContactFlow(null)}
            ...
            {/* Step Progress: CBLUE property workflow steps 3 and 4 */}
              ...
              <div className="flex items-center gap-1 mb-1">
                {Array.from({ length: 8 }, (_, index) => index + 1).map((step) => (
                  <div key={step} className={`h-1.5 flex-1 rounded-full transition-colors ${
                    step <= (contactStep === "done" ? 4 : 3) ? "bg-emerald-500" : "bg-gray-200"
                  }`} />
                ))}
              </div>
              <p className="text-xs text-gray-400 text-right mb-2">
                {locale === "th" ? `\u0e02\u0e31\u0e49\u0e19\u0e15\u0e2d\u0e19 ${contactStep === "done" ? 4 : 3} \u0e08\u0e32\u0e01 8` :
                 locale === "zh" ? `\u6b65\u9aa4 ${contactStep === "done" ? 4 : 3} / 8` :
                 `Step ${contactStep === "done" ? 4 : 3} of 8`}
              </p>
            </div>
            <div className="p-6 pt-2 overflow-y-auto flex-1">
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3 mb-5">
                <div>
                  <p className="font-bold text-gray-900">
                    {showContactFlow.title}
                  </p>
                  <p className="text-sm text-green-700 font-semibold">
                    ฿{formatPrice(showContactFlow.price)}
                    {showContactFlow.listingType === "RENT" ? "/mo" : ""}
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    {locale === "th"
                      ...
                      : locale === "zh"
                        ? "项目地点"
                        : "Site Location"}
                    : {getPropertyModalLocation(showContactFlow)}
                  </p>
                  {Array.isArray(showContactFlow.images) &&
                    showContactFlow.images.length > 0 && (
                      <button
                        type="button"
              ...
              {/* Step 3 of 8: confirm the selected listing and issue the inquiry */}
              {contactStep === "po" &&
                (() => {
                    ...
                    <div>
                      <div className="text-center mb-4">
                        <p className="text-xs text-gray-400">
                          {locale === "th"
                            ? "CBLUE ออกให้เป็นฝ่ายที่สาม"
                            : locale === "zh"
                              ? "CBLUE作为第三方签发"
                              : "Issued by CBLUE as third party"}
                        </p>
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 text-sm space-y-2">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">
                            ฿{formatPrice(showContactFlow.price)}
                            {showContactFlow.listingType === "RENT"
                              ? "/mo"
                              : ""}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-500">
                            {locale === "th"
                              ? "ระดับบริการ (กำหนดโดยผู้ลงประกาศ)"
                              : locale === "zh"
                                ? "服务等级（由房源方设定）"
                                : "Service Tier (set by lister)"}
                          </span>
                        ...
                        <div className="flex justify-between border-t border-gray-100 pt-2">
                          <span className="text-gray-600 font-semibold">
                            {locale === "th"
                              ? "ค่าดำเนินการ (ชำระในแดชบอร์ด)"
                              : locale === "zh"
                                ? "处理费（在控制台支付）"
                                : "Processing Fee (pay in Dashboard)"}
                          </span>
                      ...
                      <button
                        onClick={async () => {
                          ...
                          const activeSubscriber =
                            subscriber || readStoredSubscriber();
                          try {
                            let token =
                              localStorage.getItem("subscriber_token") || "";
                            ...
                            let inquirySession =
                              await getInquirySessionType(token);
                            if (inquirySession !== "authenticated") {
                              const refreshedToken =
                                await refreshSubscriberSession(token);
                              if (!refreshedToken) {
                                clearSubscriberSession();
                                setShowContactFlow(null);
                                setPendingContactProp(currentFlow);
                            ...
                            const submitInquiry = (authToken: string) => fetch("/api/v1/blue/property-workflow/inquiries", {
                              method: "POST",
                              headers: { "Content-Type": "application/json", Authorization: `Bearer ${authToken}` },
                              body: JSON.stringify({
                                listingId: currentFlow.id,
                                requestDetails: currentFlow.description || "",
                              }),
                            });
                            ...
                            if (!res.ok) {
                              ...
                              const msg = Array.isArray(errData?.message)
                                ...
                                : errData?.message ||
                                  (locale === "th"
                                    ? "ไม่สามารถส่งคำขอได้ กรุณาเข้าสู่ระบบใหม่แล้วลองอีกครั้ง"
                                    : locale === "zh"
                                      ? "无法发送询盘。请重新登录后再试。"
                                      : "Could not send the inquiry. Please log in again and retry.");
                              if (
                                /login|account|authenticated|unauthorized|forbidden/i.test(
                                  String(msg),
                                )
                              ) {
                                setShowContactFlow(null);
                            ...
                            const authoritativeReference = String(
                              snapshot?.reference || snapshot?.poNumber || "",
                            ).trim();
                            if (!authoritativeReference) {
                              alert(
                                locale === "th"
                                  ? "ไม่พบเลขอ้างอิงจากระบบ"
                                  : locale === "zh"
                                    ? "系统未返回询盘编号"
                                    : "The server did not return an inquiry reference.",
                              );
                              return;
                            }
                            setPoNumber(authoritativeReference);
                          } catch {
                            alert(
                              locale === "th"
                                ...
                                : locale === "zh"
                                  ? "目前无法发送询盘"
                                  : "Could not send the inquiry right now.",
                            );
                            return;
                          }
              ...
              {/* Step 4 of 8: the persisted inquiry is now awaiting lister action */}
              {contactStep === "notify" && (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    {locale === "th"
                      ? "กำลังส่งการสอบถามของคุณไปยังผู้ลงประกาศ"
                      : locale === "zh"
                        ? "正在发送您的询盘给房源方"
                        : "Sending your inquiry to the property lister"}
                  </p>
              ...
              {/* Persisted inquiry confirmation */}
              {contactStep === "done" && (
                <div className="text-center py-4">
                  ...
                  <div className="flex gap-3 justify-center">
                    ...
                    <button
                      onClick={() => setShowContactFlow(null)}
                      className="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-xl font-semibold text-sm"
                    >
                      {locale === "th"
                        ? "ค้นหาต่อ"
                        : locale === "zh"
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 mb-6">
                {locale === "th"
                  ? "ลองเปลี่ยนตัวกรอง หรือลงประกาศของคุณ"
                  : locale === "zh"
                    ? "请尝试调整筛选条件，或发布您的房产"
                    : "Try adjusting your filters, or list your own property"}
              </p>
                  ...
                  >
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getTierBadgeStyle(prop.tier)}`}
                        >
                ...
                <div>
                  <div className="text-center mb-8">
                    <h2 className="text-2xl font-bold text-gray-900">
                      {locale === "th"
                        ? "ประกาศล่าสุด 20 รายการ (ใหม่สุดก่อน)"
                        : locale === "zh"
                          ? "最新20条房源（按发布时间倒序）"
                          : "Latest 20 Listings (Newest First)"}
                    </h2>
                    <p className="text-sm text-gray-500 mt-2">
                      {locale === "th"
                        ? "แสดงผลล่าสุดก่อน เพื่อให้จับคู่ได้เร็วขึ้น"
                        : locale === "zh"
                          ? "优先显示最新发布，提升匹配效率"
                          : "Latest posts are shown first to improve matching speed."}
                    </p>
                  </div>
