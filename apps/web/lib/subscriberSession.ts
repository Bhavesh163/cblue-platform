...
export function shouldRefreshSubscriberSession(
  token: string,
  leewaySeconds = 60,
): boolean {
  ...
  return (
    expiresAt !== null &&
    expiresAt <= Math.floor(Date.now() / 1000) + leewaySeconds
  );
}
...
export async function refreshSubscriberSession(
  ...
  const refresh = async (): Promise<string | null> => {
    try {
      ...
      if (!isSubscriberRefreshCurrent(localStorage, refreshSnapshot))
        return null;
...
export async function ensureFreshSubscriberSession(
  ...
  const token = currentToken ?? localStorage.getItem("subscriber_token");
  if (!token) return null;
  return shouldRefreshSubscriberSession(token)
    ? refreshSubscriberSession(token)
    : token;
}

export async function fetchWithSubscriberSession(
  input: RequestInfo | URL,
  init: RequestInit = {},
  currentToken?: string | null,
): Promise<Response | null> {
  const token = await ensureFreshSubscriberSession(currentToken);
  if (!token) return null;
...
export async function validateSubscriberSession(
  ...
  try {
    const response = await fetchWithSubscriberSession(
      "/api/v1/users/me",
      { cache: "no-store" },
      token,
    );
    if (!response) return null;
    if (response.ok) return localStorage.getItem("subscriber_token") || token;
    if (response.status === 401 || response.status === 403) {
      clearSubscriberSession();
      return null;
    }
