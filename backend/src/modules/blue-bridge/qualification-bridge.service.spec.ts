...
describe('QualificationBridgeService', () => {
  ...
  it('rejects missing or invalid bridge keys before resolving identity', async () => {
    await expect(service.getSnapshot('user-1')).rejects.toBeInstanceOf(UnauthorizedException);
    await expect(service.getSnapshot('user-1', 'wrong-secret'))
  ...
  it('returns only the linked fixer qualification snapshot without private storage keys', async () => {
    prisma.subscriber.findFirst.mockResolvedValue({ id: 'subscriber-1', email: 'partner@example.com' });
    ...
    prisma.fixer.findFirst.mockResolvedValue({
      ...
      qualificationSubmissions: [{
        ...
        evaluations: [{
          id: 'evaluation-1', provider: 'DETERMINISTIC_POLICY', model: null,
          policyVersion: 'cblue-fixer-qualification-v1', status: 'COMPLETED',
          deterministicScore: 78, aiScore: null, risk: 'LOW',
          recommendedTier: 'STANDARD', confidence: 90,
      ...
      tierQualifications: [{
        id: 'tier-1', recommendedTier: 'STANDARD', approvedTier: null,
        source: 'DETERMINISTIC', policyVersion: 'cblue-fixer-qualification-v1',
        reason: 'Pending review', effectiveAt: null, expiresAt: null,
        createdAt: new Date('2026-07-24T00:00:00.000Z'),
      }],
    });
  ...
  it('does not expose a qualification record for an unknown linked subject', async () => {
    ...
    await expect(service.getSnapshot('unknown@example.com', 'bridge-secret'))
