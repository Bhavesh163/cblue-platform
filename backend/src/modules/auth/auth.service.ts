...
export class AuthService {
  ...
  async refreshToken(dto: RefreshTokenDto) {
    const rotated = await this.refreshSessions.rotate({
      ...
      clientId: this.firstPartyClientId(),
      audience: this.firstPartyAudience(),
    });
    return this.tokenBundle(rotated.user, {
  ...
  async logout(dto: RefreshTokenDto): Promise<void> {
    await this.refreshSessions.revokeFamily(dto.refreshToken, 'logout');
  }
  ...
  private async generateTokens(user: {
    ...
    return this.tokenBundle(user, refresh);
  }
  ...
  private async tokenBundle(
    ...
    const accessTokenExpiresAt = new Date(Date.now() + ms(expiration));
  ...
  private firstPartyAudience() {
    return this.configService.get<string>('auth.firstPartyAudience') || 'CBLUE';
  }
